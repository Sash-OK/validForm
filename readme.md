...
var options = {
            ...
            required: [{
                ...
                fieldParentClass: '.selector', /* Селектор родителя поля, к которому будет добавлен класс, если не указан то класс будет добавлен с самому полю */
                ...
                customFunc: function () {} /* Функция для проверки поля, далжна вернуть true или false */
            }],

            onSuccess: function (form) {}, /* Действие при успешной проверке всех полей, по умолчанию $(form).unbind('submit').submit(); */
            onFalue: function (form) {} /* Действие при не правильно заполненных или не заполненных полях формы */
        }
```
